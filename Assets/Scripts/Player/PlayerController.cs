...
public class PlayerController : MonoBehaviour
{
    ...
    [Tooltip("The sprite renderer for the jetpack")]
    public GameObject jetpackrenderer;
    [Tooltip("The Parent Player Object")]
    public GameObject ParentObject;
    [Tooltip("Hitbox for the Player's attack")]
    ...
    public bool attackinput
    {
        ...
        {
            if (inputManager != null)
                return inputManager.attackpressed;
            else
                return false;
        }
    }
    ...
    [Tooltip("Maximum fuel for our jetpack")]
    public float maxfuel = 100f;
    [Tooltip("Actual fuel left in our jetpack")]
    public float fuel;
    [Tooltip("The rate at which our jetpack refuels while grounded")]
    public float refuellingrate = 0.1f;
    [Tooltip("The power applied by our jetpack")]
    public float jetpackpower = 0.5f;
    [Tooltip("Time for which our attack persists")]
    public float attacktime=1f;
    [Tooltip("rate of attacks")]
    ...
    {
        if(grounded && fuel < maxfuel)
        {
            fuel = fuel + (refuellingrate*Time.deltaTime);
        }
    }
    ...
    private IEnumerator Jump(float powerMultiplier = 1.0f)
    {
        ...
        /// Code to make jetpack work
        if (inputManager.jumpHeld && timesJumped == allowedJumps && state != PlayerState.Dead)
        {
            jumping = true;
            flying = true;

            playerRigidbody.velocity = new Vector2(playerRigidbody.velocity.x, 0);
            playerRigidbody.gravityScale = jetpackpower;
            while (inputManager.jumpHeld && fuel >0)
            {
                yield return null;
                fuel = fuel - 0.07f;
   ...
   /* private void HandleSpriteDirection()
    ...
    private void ParentObjectScale()
    {
        if (!facingright && horizontalMovementInput > 0 || facingright && horizontalMovementInput<0)
        {
